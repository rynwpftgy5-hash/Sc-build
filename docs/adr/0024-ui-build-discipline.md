...
| F15 | Merge ≠ live — code committed but never deployed | On 2026-05-21 the §8.4a.25c auto-fix loop merged six PRs to `main` (Share button, link-in-new-tab, scroll preservation, learning-gap save, in-progress definition, audio download). The `/feedback` surface read these as `status='merged'` and showed "Shipped" to the user. Each PR's CI was green; each merge was real. But the Worker had not been redeployed from `main` since, so the live site at `spacesc-mcp.75xnd2784n.workers.dev` was still serving the pre-merge bundle. Campbell looked at `/uc3` for the Share button 24h later and it wasn't there. He said "this feature has failed" — and he was right, because what we'd called "shipped" was a lie of category: PR merge state, not live deploy state. | "Shipped" requires THREE facts: (1) live URL returns the new content; (2) a distinctive marker from the diff (new emoji, new function name, changed copy) is grep-able in the live HTML; (3) the `feedback_fixes.live_at` column is non-null for the row. If any fact is false, the surface shows "saved to repo · waiting for next deploy" — never "Shipped." Auto-fix workflow now runs `wrangler deploy` after merge; Worker's */2-min cron self-marks live on every boot. Source-of-truth note: deploys must come from `main` (not feature branches) so the bundle matches the merge history — and re-pull `main` before deploying. |
